...
private:
  ...
  void AddToMap(const ModuleSP &module_sp) {
    ...
    if (name.IsEmpty())
      return;
    auto &vec = m_name_to_modules[name];
  ...
  void RemoveEquivalentModulesFromMap(const ModuleSP &module_sp) {
    ...
    for (int i = vec.size() - 1; i >= 0; --i) {
      llvm::erase_if(vec, [&equivalent_module_spec](ModuleSP module) {
        return module->MatchesModuleSpec(equivalent_module_spec);
      });
    }
  }
  ...
  llvm::StringMap<llvm::SmallVector<ModuleSP, 1>> m_name_to_modules;
};
