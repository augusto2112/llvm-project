...
TEST(SwiftDWARFValidationJournal, BuildDivergenceRecord) {
  auto refl = makeRef(2147483647);
  auto dwarf = makeRef(1);
  JournalRecordInputs in;
  in.type_mangled = "$s1a3FooCD";
  in.producer = "GetClassInstanceTypeInfo";
  in.level = "strict";
  in.provider_id = 0;
  in.refl = &refl;
  in.dwarf = &dwarf;
  in.flags = TypeInfoComparison::Strict;

  auto rec = buildJournalRecord(in);
  EXPECT_EQ(*rec.getInteger("schema"), 1);
  EXPECT_EQ(*rec.getString("kind"), "divergence");
  EXPECT_EQ(*rec.getString("type_mangled"), "$s1a3FooCD");
  EXPECT_EQ(*rec.getString("type_demangled"), "a.Foo");
  EXPECT_EQ(*rec.getString("mangling_flavor"), "swift");
  EXPECT_EQ(*rec.getString("producer"), "GetClassInstanceTypeInfo");
  EXPECT_EQ(*rec.getString("level"), "strict");
  EXPECT_TRUE(*rec.getBoolean("refl_present"));
  EXPECT_TRUE(*rec.getBoolean("dwarf_present"));
  EXPECT_NE(rec.getObject("refl_ti"), nullptr);
  EXPECT_NE(rec.getObject("dwarf_ti"), nullptr);
  auto *diffs = rec.getArray("differences");
  ASSERT_NE(diffs, nullptr);
  ASSERT_EQ(diffs->size(), 1u);
}

TEST(SwiftDWARFValidationJournal, BuildAsymmetryRecord) {
  auto refl = makeRef(1);
  JournalRecordInputs in;
  in.type_mangled = "$s1a3FooCD";
  in.producer = "GetTypeInfo";
  in.level = "strict";
  in.refl = &refl;
  in.dwarf = nullptr; // Only the reflection side produced a result.
  in.flags = TypeInfoComparison::Strict;

  auto rec = buildJournalRecord(in);
  EXPECT_EQ(*rec.getString("kind"), "asymmetry");
  EXPECT_TRUE(*rec.getBoolean("refl_present"));
  EXPECT_FALSE(*rec.getBoolean("dwarf_present"));
  EXPECT_NE(rec.getObject("refl_ti"), nullptr);
  EXPECT_EQ(rec.getObject("dwarf_ti"), nullptr);
  EXPECT_EQ(rec.getArray("differences")->size(), 0u);
}

TEST(SwiftDWARFValidationJournal, WriteRecordRoundTrip) {
  llvm::SmallString<128> dir;
  ASSERT_FALSE(llvm::sys::fs::createUniqueDirectory("swiftjournaltest", dir));
  ...
  // Two records must append (not overwrite) into one per-pid file.
  writeJournalRecord(dir, llvm::json::Value(buildJournalRecord(in)));
  writeJournalRecord(dir, llvm::json::Value(buildJournalRecord(in)));
  ...
  auto parsed = llvm::json::parse(lines[0]);
  ASSERT_TRUE(static_cast<bool>(parsed));
  auto *obj = parsed->getAsObject();
  ASSERT_NE(obj, nullptr);
  EXPECT_EQ(*obj->getInteger("schema"), 1);
  EXPECT_EQ(*obj->getString("kind"), "divergence");
